...
regress: build_images
	docker compose -f test/regress/docker-compose.yaml down && MDB_BRANCH=MDB_${POSTGRES_VERSION} docker compose -f test/regress/docker-compose.yaml build --build-arg POSTGRES_VERSION=${POSTGRES_VERSION} --build-arg image=${image} --build-arg codename=${codename} && docker compose -f test/regress/docker-compose.yaml run --remove-orphans regress
