...
func (ctsa *CachedTSAChecker) CheckTSA(sh shard.ShardHostInstance, TSATimeout time.Duration) (CachedCheckResult, error) {
	...
	* much for troubles here, so we are fine. */
	cr, err := ctsa.innerChecker.CheckTSA(sh, TSATimeout)
	if err != nil {
