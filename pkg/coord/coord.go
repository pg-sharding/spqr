	...
	for _, change := range changes {
		...
		case topology.GenericOptionActionAdd:
			if optionValueCount > 0 {
				return fmt.Errorf("option \"%s\" with value \"%s\" provided more than once", change.Name, change.Arg)
			}
		...
		case topology.GenericOptionActionSet:
			if optionNameCount == 0 {
				return fmt.Errorf("option \"%s\" not found", change.Name)
			}
			if optionNameCount > 1 {
				return fmt.Errorf("option \"%s\" has multiple values provided", change.Name)
			}
		...
		case topology.GenericOptionActionDrop:
			...
			if change.Arg == "" {
				...
				if optionValueCount == 0 {
					return fmt.Errorf("option \"%s\" with value \"%s\" not found", change.Name, change.Arg)
				}
