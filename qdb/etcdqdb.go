...
func (q *EtcdQDB) TryCoordinatorLock(ctx context.Context) error {
	...
	op := clientv3.OpPut("/"+coordLockKey, net.JoinHostPort(config.CoordinatorConfig().Host, config.CoordinatorConfig().CoordinatorPort), clientv3.WithLease(clientv3.LeaseID(leaseGrantResp.ID)))
