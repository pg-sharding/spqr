...
func (q *MemQDB) AddShardingRule(ctx context.Context, rule *ShardingRule) error {
	spqrlog.Logger.Printf(spqrlog.LOG, "adding sharding rule %v", rule.Colnames)
	q.mu.Lock()
	defer q.mu.Unlock()
...
func (q *MemQDB) DropShardingRuleAll(ctx context.Context) error {
...
func (q *MemQDB) DropKeyRangeAll(ctx context.Context) error {
	...
	// q.krs = make(map[string]*KeyRange)
	// q.locks = make(map[string]*sync.RWMutex)
