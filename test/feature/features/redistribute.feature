Feature: Redistribution test
  ...
  Scenario: REDISTRIBUTE KEY RANGE fails if cannot await pids within timeout
    When I execute SQL on host "coordinator"
    """
    CREATE KEY RANGE kr1 FROM 0 ROUTE TO sh1 FOR DISTRIBUTION ds1;
    """
    ...
    And SQL error on host "coordinator" should match regexp
    """
    ERROR: failed to move keys: (failed to await virtual transactions to exit: .*|recoverable transfer error occurred) \(SQLSTATE SPQR[Tt]\)
    """
    When I run SQL on host "coordinator"
    """
