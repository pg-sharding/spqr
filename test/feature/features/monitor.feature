Feature: spqr-monitor test
  ...
  Scenario: spqr-monitor recover does nothing when no move task in progress
   When I run SQL on host "router"
    ...
    When I run command on host "coordinator" with timeout "30" seconds
    """
    /spqr/spqr-monitor recover --etcd-addr regress_qdb_0_1:2379 --coordinator-addr regress_coordinator:7003 -c /spqr/test/feature/conf/shard_data.yaml 2&> output.txt
    """
    Then command return code should be "0"
    When I run command on host "coordinator"
    """
  ...
  Scenario: spqr-monitor recover dry run works
   When I run SQL on host "router"
    ...
    When I run command on host "coordinator" with timeout "30" seconds
    """
    /spqr/spqr-monitor recover --etcd-addr regress_qdb_0_1:2379 --coordinator-addr regress_coordinator:7003 -c /spqr/test/feature/conf/shard_data.yaml --dry-run 2&> output.txt
    """
    Then command return code should be "0"
    When I run command on host "coordinator"
    """
  ...
  Scenario: spqr-monitor check works on router with shard_data credentials with corruptions
    When I run SQL on host "router"
    """
    CREATE TABLE xMove(w_id INT, s TEXT);
    insert into xMove(w_id, s) values(11, '002');
    ...
    When I run command on host "coordinator" with timeout "30" seconds
    """
    /spqr/spqr-monitor check --etcd-addr regress_qdb_0_1:2379 --file /tmp/report.txt -c /spqr/test/feature/conf/shard_data.yaml --tablesample-size 100 --host regress_router
    """
    Then command return code should be "0"
    And command output should match regexp
    """
