Feature: Move test
  ...
  Scenario: MOVE KEY RANGE works with many tables and foreign key constraints
    When I run SQL on host "shard1"
    """
    CREATE TABLE xMove(w_id INT, pkey INT PRIMARY KEY, s TEXT);
    insert into xMove(w_id, pkey, s) values(1, 1, '001');
    CREATE TABLE xMove2(w_id INT, skey INT REFERENCES xMove ( pkey ), s TEXT);
    insert into xMove2(w_id, skey, s) values(2, 1, '002');
    """
    Then command return code should be "0"
    When I run SQL on host "shard2"
    """
