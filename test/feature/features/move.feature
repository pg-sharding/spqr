Feature: Move test
  ...
  Scenario: MOVE KEY RANGE works with varchar keys
    ...
    CREATE KEY RANGE krid4 FROM aa ROUTE TO sh2 FOR DISTRIBUTION ds2;
    CREATE KEY RANGE krid3 FROM a ROUTE TO sh1 FOR DISTRIBUTION ds2;
  ...
  Scenario: Move nonexistent key range fails
    ...
    And SQL error on host "coordinator" should match regexp
    """
    failed to acquire key range operation lock: failed to acquire key range operation lock for key range .*krid3.*: key range does not exist \(SQLSTATE SPQRt\)
    """
  ...
  Scenario: MOVE KEY RANGE works with uuid keys
    ...
    CREATE KEY RANGE krid4 FROM '80000000-0000-0000-0000-000000000000' ROUTE TO sh2 FOR DISTRIBUTION ds2;
