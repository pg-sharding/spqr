...
type ProxyQrouter struct {
	...
	dataShards  map[string]*config.Shard
	worldShards map[string]*config.Shard
}
...
func (qr *ProxyQrouter) ListDataShards(ctx context.Context) []*datashards.DataShard {
	...
	for id, cfg := range qr.dataShards {
...
func (qr *ProxyQrouter) DropAllShardingRules(ctx context.Context) ([]*shrule.ShardingRule, error) {
	rules, err := qr.qdb.DropAllShardingRules(ctx)
	if err != nil {
		return nil, err
	}
...
func (qr *ProxyQrouter) DropAllKeyRanges(ctx context.Context) ([]*kr.KeyRange, error) {
	qr.mu.Lock()
	defer qr.mu.Unlock()

	spqrlog.Logger.Printf(spqrlog.LOG, "dropping all key range")
	resp, err := qr.qdb.DropAllKeyRanges(ctx)
...
// TODO MoveKeyRange
// TODO there are two move key range methods, it is a bit confusing
...
func (qr *ProxyQrouter) MergeKeyRanges(ctx context.Context, req *kr.MergeKeyRange) error {
	var krRight *qdb.KeyRange
	var krleft *qdb.KeyRange
	var err error

	if _, err = qr.qdb.LockKeyRange(ctx, req.KeyRangeIDLeft); err != nil {
		return err
	}
...
func (qr *ProxyQrouter) SplitKeyRange(ctx context.Context, req *kr.SplitKeyRange) error {
...
func (qr *ProxyQrouter) UnlockKeyRange(ctx context.Context, krid string) error {
