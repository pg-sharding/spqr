...
func prepareTestCheckTableIsRoutable(t *testing.T) (*qrouter.ProxyQrouter, error) {
	...
	if err = db.ExecNoTransaction(ctx, statements); err != nil {
		return nil, err
	}
...
// TestPsqlMetaCommandsRouting verifies that SQL queries generated by psql
// meta-commands (\d+, \dt+, \dp+) are routed via RandomDispatchPlan,
// since they only access pg_catalog tables.
func TestPsqlMetaCommandsRouting(t *testing.T) {
	assert := assert.New(t)
	...
	for _, tt := range []tcase{
		{
			desc: `\dt+ (list tables with details)`,
		...
		},
		{
			...
			query: `SELECT n.nspname as "Schema", c.relname as "Name",
				...
				FROM pg_catalog.pg_class c
				LEFT JOIN pg_catalog.pg_namespace n ON n.oid = c.relnamespace
				WHERE c.relkind IN ('r', 'v', 'S') AND n.nspname NOT LIKE 'pg_%'
				ORDER BY 1, 2`,
			exp: &plan.RandomDispatchPlan{},
		},
		{
			desc: `\d+ table info (main query)`,
			query: `SELECT c.relchecks, c.relkind, c.relhasindex, c.relhasrules,
				...
				WHERE c.oid = 'r'::regclass`,
			exp: &plan.RandomDispatchPlan{},
		},
		{
			desc: `\d+ column details`,
		...
		{
			desc: `\d+ index details`,
			query: `SELECT c2.relname, i.indisprimary, i.indisunique, i.indisclustered, i.indisvalid,
				pg_catalog.pg_get_indexdef(i.indexrelid, 0, true),
				pg_catalog.pg_get_constraintdef(con.oid, true), con.contype,
				con.condeferrable, con.condeferred, i.indisreplident, c2.reltablespace
				FROM pg_catalog.pg_class c, pg_catalog.pg_class c2, pg_catalog.pg_index i
				LEFT JOIN pg_catalog.pg_constraint con ON (con.conrelid = i.indrelid
					AND con.conindid = i.indexrelid AND con.contype IN ('p','u','x'))
				WHERE c.oid = 'r'::regclass AND c.oid = i.indrelid AND i.indexrelid = c2.oid
		...
		assert.NoError(err, "parse %s", tt.desc)
		...
		assert.NoError(err, "plan %s", tt.desc)

		assert.Equal(tt.exp, tmp, tt.desc)
	}
}
