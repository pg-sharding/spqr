...
func (b *BalancerImpl) getCurrentTaskFromQDB(ctx context.Context) (group *tasks.BalancerTask, err error) {
	...
	resp, err := tasksService.GetBalancerTask(ctx, &protos.GetBalancerTaskRequest{})
	if err != nil {
		return nil, err
	}
...
func (b *BalancerImpl) executeTasks(ctx context.Context, task *tasks.BalancerTask) error {
	...
	for {
		...
		case tasks.BalancerTaskMoved:
			...
			if _, err = taskService.RemoveBalancerTask(ctx, &protos.RemoveBalancerTaskRequest{}); err != nil {
				return err
			}
			return nil
		default:
