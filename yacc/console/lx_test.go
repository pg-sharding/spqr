...
func TestSimpleLex(t *testing.T) {
	...
	for _, tt := range []tcase{
		...
		{
			query: `CREATE SHARD sh1 OPTIONS (HOST "localhost:6432", HOST "other_host:7432")`,
			exp: []int{
				spqrparser.CREATE,
				spqrparser.SHARD,
				spqrparser.IDENT,
		...
		{
			query: `CREATE SHARD sh1 WITH HOSTS "localhost:6432", "other_host:7432" OPTIONS (dbname db1, user user1, password password)`,
			exp: []int{
				spqrparser.CREATE,
				spqrparser.SHARD,
				spqrparser.IDENT,
		...
		{
			query: `ALTER SHARD sh1 HOSTS "anotherhost:6432"`,
			exp: []int{
				spqrparser.ALTER,
				spqrparser.SHARD,
				spqrparser.IDENT,
