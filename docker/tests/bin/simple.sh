...
psql "host=spqr_router_1_1 sslmode=disable user=user1 dbname=db1 port=6432" -c 'insert into x values(1)' || {
	echo "ERROR: tests failed"
	exit 1
}

psql "host=spqr_router_1_1 sslmode=disable user=user1 dbname=db1 port=6432" -c 'insert into x values(11)' || {
	echo "ERROR: tests failed"
	exit 1
}
