#!/bin/bash
set -ex

POSTGRES_VERSION=$1
if [ "x" == "${POSTGRES_VERSION}x" ]
then
    echo "no postgres version specified"
    exit 1
fi

rm -fr /var/lib/postgresql/$POSTGRES_VERSION/main/ 

if [ "x" != "x$PG_MASTER" ]; then
    PG_SLOT_NAME=${PG_SLOT_NAME:-replication_slot}
    until sudo -u postgres psql -h $PG_MASTER -d postgres -p 6432 -c "SELECT pg_drop_replication_slot('$PG_SLOT_NAME');"; sudo -u postgres psql -h $PG_MASTER -d postgres -p 6432 -c "SELECT pg_create_physical_replication_slot('$PG_SLOT_NAME');"
    do
        echo 'Waiting for replication slot creation'
        sleep 1s
    done
    until sudo -u postgres pg_basebackup --pgdata=/var/lib/postgresql/$POSTGRES_VERSION/main -R --slot=$PG_SLOT_NAME --host=$PG_MASTER --port=6432
    do
        echo 'Waiting for primary to connect...'
        sleep 1s
    done
    sudo -u postgres chmod 0700 /var/lib/postgresql/$POSTGRES_VERSION/main
    sudo -u postgres /usr/lib/postgresql/$POSTGRES_VERSION/bin/postgres -D /var/lib/postgresql/$POSTGRES_VERSION/main
else
    sudo -u postgres /usr/lib/postgresql/$POSTGRES_VERSION/bin/pg_ctl -D /var/lib/postgresql/$POSTGRES_VERSION/main/ init
    setup
    if  [ "x" != "x$PG_SYNC_REPLICA" ]; then
        sudo -u postgres /usr/lib/postgresql/$POSTGRES_VERSION/bin/postgres -c "synchronous_standby_names=FIRST 1 (\"walreceiver\")" -D /var/lib/postgresql/$POSTGRES_VERSION/main
    else
        sudo -u postgres /usr/lib/postgresql/$POSTGRES_VERSION/bin/postgres -D /var/lib/postgresql/$POSTGRES_VERSION/main
    fi
fi
