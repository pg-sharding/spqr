...
- `spqrguard` is added to `shared_preload_libraries` in `postgresql.conf`.
  For an existing cluster, follow the complete
  [spqrguard rollout guide](/deployment/spqrguard).
- The coordinator has `use_spqrguard: true`. The
  `forbid_direct_shard_queries` setting is not required for data transfers;
  enable it only when direct writes to shards must also be blocked.
