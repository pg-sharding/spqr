...
2. **Cardinality Considerations**: For [direct key sharding](/sharding/ranged), consider using a lower-cardinality column first (e.g., tenant ID) followed by a higher-cardinality column (e.g., user ID or timestamp).
