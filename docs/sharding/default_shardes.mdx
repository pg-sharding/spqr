...
Distribution can have a default shard. It means that if router matches query to distribution with default shard it definetelly find route. It route will be to this shard. 
...
## How to use it?
We have the distribytion with key range "From 0". Try alter distribution with default shard command:
```sql
ALTER DISTRIBUTION testWithDefault DEFAULT SHARD sh2;
...
-------------------------+----------+-----------------+----------------------
 kr18b713215eae          | sh1      | ds1             | 0
 testWithDefault.DEFAULT | sh2      | testWithDefault | -9223372036854775808
(2 rows)

```
Default shards are supporting only `integer` and `varchar` types now.