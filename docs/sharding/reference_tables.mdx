...
Whenever router need new autoincrement value it ask it from coordinator. To improve perfomance generating autoincrement values you can set `identity_range_size` in router config file. This parameter set size of range identifiers which router get  from coordinator in a time.