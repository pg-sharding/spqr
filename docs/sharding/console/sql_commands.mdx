...
This command is used to create a [reference table](/sharding/reference_tables), which is a table that is replicated across all shards in the cluster. Optionally, you can specify an auto-increment column for the table using the AUTO INCREMENT clause.

```sql
CREATE REFERENCE TABLE <table_name> [AUTO INCREMENT <column_name>]
```
