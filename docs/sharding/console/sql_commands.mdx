...
This command is used to add a default shard to an existing distribution.

```sql
ALTER DISTRIBUTION <distributionID> ADD DEFAULT SHARD <shardId>

where <shardId> is a default shard identifier. The shard must exist.
```

### ALTER DISTRIBUTION DROP DEFAULT SHARD
This command is used to remove the default shard from an existing distribution.
