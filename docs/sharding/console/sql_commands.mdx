...
<Warning>If you don’t fully understand how SPQR works, you should not use this command in production. Use REDISTRIBUTE instead. </Warning>
...
You can also use optional keywords `CHECK`, `NOWAIT` and `APPLY`. By default, command will check for the ability to move keys, then perform the actual movement. With `CHECK`, you can perform a dry-run.
With `APPLY`, you can skip checks. With `NOWAIT` you will NOT wait for data move completion, but you will get an error like in `CHECK` case if anything is misconfigured.

```sql
REDISTRIBUTE KEY RANGE <keyRangeID> TO <shardID> [ BATCH SIZE <batchSize> ] [ CHECK | APPLY | NOWAIT ];
```
...
where entity is one of backend_connections, clients, coordinator_address, databases, distributions, hosts, instance, is_read_only, key_ranges, key_ranges_extended, move_stats, move_task, pools, prepared_statements, reference_relations, relations, routers, sequences, sharding_rules, shards, status, task_group, time_quantiles, tsa_cache, users, version:
```
