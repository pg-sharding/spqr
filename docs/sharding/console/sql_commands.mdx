...
where entity is one of backend_connections, clients, coordinator_address, databases, distributions, hosts, instance, is_read_only, key_ranges, move_stats, pools, prepared_statements, reference_relations, relations, routers, sequences, sharding_rules, shards, status, task_groups, time_quantiles, tsa_cache, users, version:
```
