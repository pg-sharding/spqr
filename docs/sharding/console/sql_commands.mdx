...
`BATCH SIZE` is an integer value that controls the number of keys moved in each batch during redistribution.

```sql
REDISTRIBUTE KEY RANGE <keyRangeID> TO <shardID> BATCH SIZE <batchSize>;
```
