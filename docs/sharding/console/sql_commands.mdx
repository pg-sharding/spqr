...
Replaces the entire distribution key of an already attached relation. Unlike `RENAME DISTRIBUTION COLUMN`, this does not validate against the existing key — it overwrites column names and hash functions in one shot.
...
Renames a column in the distribution key of an attached relation. This is useful when a relation was attached with an incorrectly cased column name (e.g., `SITEID` instead of `siteid`), causing SPQR to fail routing queries because PostgreSQL normalizes unquoted identifiers to lowercase.
...
```sql
-- Fix a single column name
ALTER DISTRIBUTION ds1 ALTER RELATION users RENAME DISTRIBUTION COLUMN SITEID TO siteid;

-- Fix multiple columns in a composite key (one at a time)
ALTER DISTRIBUTION ds1 ALTER RELATION events RENAME DISTRIBUTION COLUMN RegionID TO region_id;
ALTER DISTRIBUTION ds1 ALTER RELATION events RENAME DISTRIBUTION COLUMN EventID TO event_id;
```

<Warning>
This command does not have well-defined behavior with concurrent read-write operations. Use it during setup or maintenance windows only. Verify that old and new column names still route data to the correct shards before resuming traffic.
</Warning>
