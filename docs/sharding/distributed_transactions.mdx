...
[query_routing.default_route_behaviour](/configuration/router#query-routing-settings) parameter determines what happens when SPQR cannot route a query to a single shard:
...
DROP DATABASE prod;
-- ERROR: client processing error: query is blocked due to the default_route_behavior.
