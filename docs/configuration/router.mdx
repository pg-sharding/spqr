...
| `client_tls`         | TLS for outgoing router connections to the coordinator gRPC API. | Object of `GRPCClientTLSConfig` | disabled |
| `server_tls`         | TLS for the router gRPC API.                      | Object of `GRPCServerTLSConfig` | disabled |
...
`client_tls` and `server_tls` support `disabled`, `tls`, and `mtls` modes. Client configuration accepts `root_ca_file` and an optional `server_name`; `cert_file` and `key_file` are required in `mtls` mode. Server configuration requires `cert_file` and `key_file` for TLS, plus `client_ca_file` in `mtls` mode.
