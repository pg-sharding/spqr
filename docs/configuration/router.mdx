...
|---------|-------------|---------|
| `grpc_api_tls` | Server credentials for the router gRPC API used by coordinators. | `disabled` |
| `coordinator_grpc_tls` | Client trust and identity for router-to-coordinator gRPC connections. | `disabled` |

These settings are directional and may use different certificates or trust roots. See [Internal gRPC TLS](./grpc_tls) for modes, fields, CLI options, and rollout guidance.
