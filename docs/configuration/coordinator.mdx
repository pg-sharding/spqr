...
| `client_tls`               | TLS for outgoing internal gRPC connections. See [gRPC TLS Settings](#grpc-tls-settings).      | Object of `GRPCClientTLSConfig`               | disabled |
| `server_tls`               | TLS for the internal gRPC API. See [gRPC TLS Settings](#grpc-tls-settings).                   | Object of `GRPCServerTLSConfig`               | disabled |
...
| `watch_routers_qdb`        | Whether to check router's QDB contents and perform automatic re-bootstrapping. | `true`, `false` | `false` |
...
4. Syncs coordinator address if needed
5. Sync router QDB metadata if enabled via `watch_routers_qdb`
6. Opens/closes routers in QDB based on their status
7. Cleans up connections for removed routers
8. Sleeps for `iteration_timeout` before the next cycle
