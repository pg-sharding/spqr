...
| `router_keepalive_time`    | Interval for sending gRPC keepalive pings to routers. Prevents idle connection closure by network intermediaries. Default `30s`. | Duration string (for example `15s`, `30s`, `1m`) |
| `router_keepalive_timeout` | Time to wait for keepalive ping response before considering connection dead. Default `20s`.  | Duration string (for example `10s`, `20s`)    |
