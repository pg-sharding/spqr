...
| `client_tls`               | TLS for outgoing coordinator connections to router gRPC APIs.                                | Object of `GRPCClientTLSConfig`               | disabled |
| `server_tls`               | TLS for the coordinator gRPC API.                                                            | Object of `GRPCServerTLSConfig`               | disabled |
...
| `use_spqrguard`            | Maintains [`spqrguard`](/deployment/spqrguard) metadata for key ranges and distributed and reference tables during topology changes and data moves. | `true`, `false` | `false` |
| `forbid_direct_shard_queries` | Enables global `spqrguard` protection against direct writes to distributed and reference tables. | `true`, `false` | `false` |

## gRPC TLS Settings

Both settings default to disabled when omitted. They are read from the same coordinator configuration when running either `spqr-coordinator` or an embedded coordinator through `spqr-router --coordinator-config`.

`client_tls` configures connections initiated by the coordinator to router gRPC APIs:

```yaml
client_tls:
  mode: tls # disabled, tls, or mtls
  root_ca_file: /path/to/ca.crt
  server_name: router.internal # optional certificate name override
  # cert_file and key_file are required for mtls
```

`server_tls` configures the coordinator gRPC API:
