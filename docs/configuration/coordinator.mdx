...
|---------|-------------|---------|
| `grpc_api_tls` | Server credentials for the coordinator gRPC API used by routers and tools. | `disabled` |
| `router_grpc_tls` | Client trust and identity for coordinator-to-router gRPC connections. | `disabled` |

These settings are independent because the coordinator is a server in one direction and a client in the other. See [Internal gRPC TLS](./grpc_tls) for modes, certificate requirements, and migration guidance.
