...
Create configuration files for each SPQR Infra instance.
...
<Note>
The etcd connection is configured via command-line arguments when starting the router. See the startup commands below for details.
</Note>

**Instance 2 configuration (spqr-infra2.yaml):**

<Note>
Instances 2 and 3 use the same configuration as Instance 1, except for the port numbers. Keep the frontend and backend rules synchronized across all instances to ensure consistent behavior.
</Note>
...
Start each instance with its configuration and etcd connection details:
...
<Note>
In this Docker Compose example, all SPQR instances use the same internal ports (6432, 7432) but are mapped to different host ports (6432-6434, 7432-7434). Each instance uses the same configuration file since they run in separate containers.
</Note>
...
services:
  ...
  spqr1:
    ...
    ports:
      - "6432:6432"  # Host port 6432 -> Container port 6432
      - "7432:7432"  # Host port 7432 -> Container port 7432
    volumes:
  ...
  spqr2:
    ...
    command: spqr-router run --config /etc/spqr/config.yaml
    ports:
      - "6433:6432"  # Host port 6433 -> Container port 6432
      - "7433:7432"  # Host port 7433 -> Container port 7432
    volumes:
      - ./spqr-infra.yaml:/etc/spqr/config.yaml
    depends_on:
      - etcd1
  ...
  spqr3:
    ...
    command: spqr-router run --config /etc/spqr/config.yaml
    ports:
      - "6434:6432"  # Host port 6434 -> Container port 6432
      - "7434:7432"  # Host port 7434 -> Container port 7432
    volumes:
      - ./spqr-infra.yaml:/etc/spqr/config.yaml
    depends_on:
      - etcd1
...
**Shared configuration file (spqr-infra.yaml):**
```yaml spqr-infra.yaml
host: '0.0.0.0'

# All instances use the same internal ports
router_port: '6432'
admin_console_port: '7432'
grpc_api_port: '7010'
