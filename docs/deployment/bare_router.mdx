...
- A bare router does not make router-to-coordinator gRPC connections. Leave `coordinator_grpc_tls` disabled. Configure `grpc_api_tls` only if another trusted component or tool needs the router gRPC API.
