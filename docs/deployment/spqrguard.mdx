...
This SPQR version accepts only extension version `2.3`. The `spqrguard` repo
...
- Backups of configs, QDB metadata, and affected databases.

<Warning>
Don't change topology, create or drop tables, or move key ranges during this
procedure. Wait for active task groups to finish first.
</Warning>
...
You'll use the relation lists to verify metadata in step 6. You'll need the key
ranges because step 5 won't backfill existing ones.

Pick one distributed table for testing in step 8. If available, pick one
reference table too. Avoid tables with statement-level triggers or external
side effects.
...
For an HA shard that must stay writable, restart replicas first. Switch over to
...
Do not leave any instance without the module. It would be unprotected after
failover.
...
If an older version is installed and an update path to `2.3` exists:
...
Restart routers first, one at a time. Then restart coordinators. The active
coordinator pushes relation metadata to every shard. In HA setups, restart all
coordinator instances. Only the lock holder performs setup.
...
Run this on every routed database on every shard. It should return no enabled
rows:
...
Run this directly on every routed database on every shard:
...
Compare the result with your baseline from step 1. Every SPQR-managed table in
the database must appear here. Investigate differences before continuing.

### Backfill existing key ranges

`use_spqrguard` maintains future key range changes. It doesn't backfill
existing key ranges. For each key range in your baseline, insert it into its
owning shard.
...
<Warning>
`'TODO'` and `67` are literal values written by SPQR. They are not placeholders.
Replace only the key range IDs in the `VALUES` list.
</Warning>

Run a separate `INSERT` for each shard. Add only the key ranges it owns.
...
The result must match that shard's key ranges in your baseline. Investigate
differences instead of deleting metadata.
...
Connect directly to each shard primary. Run a no-op write on the distributed
table picked in step 1. Replace `your_schema.your_distributed_table` with its
qualified name:
...
Repeat for the reference table picked in step 1. Replace
...
Keep `use_spqrguard` on so SPQR continues maintaining metadata. Removing the
