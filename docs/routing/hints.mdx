...
The __spqr__auto_distribution parameter is used to automatically determine what distribution to use when possible.
```sql
SELECT * FROM users WHERE name = 'denchick'
/* __spqr__auto_distribution: true,__spqr__sharding_key: 123 */;
```
