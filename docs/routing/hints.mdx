...
- `smart-read-write`: Connects to a server in read-write mode, and fails if the primary is unavailable. Automacitly routes read-only queries to standbys (with prefer-standby semantics), if query_routing: auto_route_ro_on_standby is set to true in configure file.
