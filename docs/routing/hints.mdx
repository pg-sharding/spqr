...
Distributions with a single key column are now supported. The following key types are supported for automatic creation of distributed tables: integer, UUID, and varchar. Hashed distributions are currently not supported now for autocreate tables.
```sql
...
`2pc` mode only works when the `__spqr_engine_v2` hint is enabled. Additionally, `2pc` requires setting the [max_prepared_transactions](https://www.postgresql.org/docs/current/runtime-config-resource.html#GUC-MAX-PREPARED-TRANSACTIONS) parameter on all shards that take part in a transaction.
```sql
