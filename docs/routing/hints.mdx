...
### __spqr__engine_v2
Enables v2 routing engine features.
...
`2pc` mode only works when the `__spqr_engine_v2` hint is enabled. Additionally, `2pc` requires setting the `max_prepared_transactions` parameter on all shards that take part in a transaction (refer to the PostgreSQL documentation).
```sql
