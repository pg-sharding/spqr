...
The `__spqr_auto_distribution__` parameter is used to automatically add a new table to the SPQR metadata without having to go into the spqr console. This parameter can only be used with the `__spq_auto_dist__` command.
