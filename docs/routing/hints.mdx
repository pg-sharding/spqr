...
Possible values:
- `v2` - When set, this value automatically enables the `query_routing.enhanced_multishard_processing` feature (it does not directly change `query_routing.engine_default`).
