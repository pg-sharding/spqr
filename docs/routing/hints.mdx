...
Virtual parameters and hint effect context differes depending on how user use them.
Generally, virtual parameters honors transaction blocks and bound, and also support savepoint feature.
So, defining virtual parameter using sql comment in query text will take effect only on statement level.
Defining virtual parameter using SET statement will take effect in transaction block bound. If no explicit transaction started, 
virtual parameter value will be session-wide.
