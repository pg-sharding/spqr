(() => {
  ...
  const moveMarkup = (move) => `<a class="cluster-move ${move.state.toLowerCase()}" href="${demo.url(move.state === 'ERROR' ? 'task.html' : 'range.html', { distribution: move.distributionIndex, id: move.rangeId })}">
  ...
  if (page === 'cluster') {
    ...
    document.querySelector('[data-shard-cloud]').innerHTML = Array.from({ length: demo.shardCount }, (_, shardIndex) => `<a href="${demo.url('shard.html', { shard: demo.shardId(shardIndex) })}" style="--shard-color:${shardColor(shardIndex)}"><strong>${demo.shardId(shardIndex)}</strong><span>${demo.shardDistributions(shardIndex).length} distributions</span></a>`).join('');
    const firstShard = document.querySelector('[data-first-shard]');
    if (firstShard) firstShard.href = demo.url('shard.html', { shard: demo.shardId(0) });
