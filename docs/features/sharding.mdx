...
A [default shard](/sharding/default_shards) provides a catch-all fallback for queries whose sharding key doesn't match any explicitly defined key range.
...
See [Default Shard](/sharding/default_shards) for more details.