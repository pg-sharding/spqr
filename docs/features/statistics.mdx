...
------------------------+--------------
 ...
 router                 | 125ms
 shard.Total            | 4.891s
 shard.LockKeyRange     | 45ms
 shard.CopyData         | 4.632s
 shard.UnlockKeyRange   | 12ms
 qdb.Total              | 218ms
 qdb.UpdateKeyRange     | 156ms
 qdb.CommitMetadata     | 62ms
(9 rows)
```
...
- `shard.*`: Time spent on shard operations (locking, copying data, unlocking)
- `qdb.*`: Time spent on metadata operations in the query database
